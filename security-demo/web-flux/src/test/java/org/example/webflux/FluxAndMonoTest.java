...
import org.example.webflux.common.config.CustomException;
...
public class FluxAndMonoTest {
    ...
    void setUp(){
        ...
        customExceptionMono = new CustomException("Flux");
    }
    ...
    @Test @DisplayName("Flux just() sample")
    void monoJustTest(){
        //React Stream 에서는 Data, Event, Signal 중에서 Signal을 사용한다.
        //onNext, onComplete, onError
        List<Signal<Integer>> list = new ArrayList<>(4);
        final Integer[] result = new Integer[1];
        Mono<Integer> mono = Mono.just(1).log().doOnEach(integerSignal -> {
            list.add(integerSignal);
            System.out.println("시그널 : "+integerSignal);
        });
