...
class CrimeService:
    ...
    def save_cctv_per_population(self) -> None:
        ...
        외국인비율 과 CCTV 상관계수 [[ 1.         -0.13607433] 거의 무시될 수 있는
                                    [-0.13607433  1.        ]]                        
         """
        cctv_per_populations.to_csv(f'{self.data.sname}cctv_per_poppulations.csv')

    def save_crime_arrest_normal(self) -> None:
        cctv = self.cctv_dataframe()
        cctv.rename(columns={cctv.columns[0]: '구별'}, inplace=True)
        cctv.drop(['2013년도 이전', '2014년', '2015년', '2016년'], axis=1, inplace=True)
        ic(cctv)
        ...
        reader = Reader()
        crime = reader.csv(f'{self.data.dname}crime_in_seoul')
        ...
        # 관서명을 구별로 변환하는 함수
        def convert_station_to_gu(polname):
            if polname in exceptions:
                return exceptions[polname]
            if polname.endswith('서'):
                return polname[:-1] + '구'
            return polname

        crime['관서명'] = crime['관서명'].apply(convert_station_to_gu)
        ...
        crime['검거율'] = (crime['살인 검거']+crime['강도 검거']+crime['강간 검거']+crime['절도 검거']+crime['폭력 검거'])/(crime['살인 발생']+crime['강도 발생']+crime['강간 발생']+crime['절도 발생']+crime['폭력 발생']) * 100
        ic(crime['검거율'])
        cctv_per_crime = pd.merge(cctv, crime)
        cor = np.corrcoef(cctv_per_crime['검거율'], cctv_per_crime['소계'])
        print(cor)

        # 병합된 데이터프레임 저장
        cctv_per_crime.to_csv(f'{self.data.sname}cctv_per_crime.csv', sep=',', na_rep="NaN", encoding='UTF-8')
...
if __name__ == "__main__":
    ...
    # service.folium_test()
    # service.save_crime_arrest_normal()
